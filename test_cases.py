...
def rotate_section(point_cloud, theta_x, theta_y, theta_z):
    result = rotate_section_x(point_cloud, theta_x)
...
def rotate_section_x(point_cloud, theta):
    S, C = np.sin(theta), np.cos(theta)
    rotation_matrix = np.array( [[1, 0, 0],
                                [0, C, -1*S],
                                [0, S, C]])
    
    new_point_cloud = np.dot(point_cloud, rotation_matrix)
    
    return new_point_cloud
    
    
def rotate_section_y(point_cloud, theta):
    S, C = np.sin(theta), np.cos(theta)
...
def rotate_section_z(point_cloud, theta):
    S, C = np.sin(theta), np.cos(theta)
...
def main():
    ...
    result = rotate_section(pcpds.get_point_cloud(), 20, 52, 93)
    print("Result:\n", result)
